    ...
    <v-form @submit.prevent="updatePatient">
        ...
        <!-- columna de usuario -->
          <v-col cols=8>
            ...
            <v-row>
                ...
                <v-btn v-show="!disabled" dark type="submit">
                  Guardar
                </v-btn>
      ...
      >
        <v-card>
          ...
          <v-card-text>Esta acción no se podrá deshacer y se perderán todos los datos del mismo.</v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn
...
export default {
  ...
  methods: {
    ...
    async updatePatient() {
      ...
      const res = await this.$axios.put(`patients/${this.$route.params.id}`, patient)
      if(res.status === 200){
